...
function renderHtmlPriceSize() {
    var qtys;
    qtys = nbd_window.jQuery('.quantity .input-text').val();
    setTimeout(function(){
        var scope = jQuery('[ng-controller="optionCtrl"]').scope();
        console.log(scope,1);
        if(scope === undefined) return;
        var sumtotal = scope.convert_to_wc_price( scope.total_cart_price);
        jQuery('.price-final_sc').text(scope.convert_to_wc_price( sumtotal ));
    },1);
}
...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document', '$interval',
    ...
    if(typeof apne_nbd_js_config != 'undefined'){
        $scope.createImage = function(){
            var formData = new FormData();
            formData.append("msg", 'animal');
            
            var config = {
                transformRequest: angular.identity,
                transformResponse: angular.identity,
                headers: {
                    'Authorization': ' Bearer 109908e0-ccfe-4a1c-9c0c-8abbd4cc3e76',
    ...
    console.log("WE ARE BEFORE UNDEFINED ");
    if(typeof apne_nbd_js_config != 'undefined'){
        $scope.apiurl = '';
        $scope.apimsg = '';
        $scope.nextImages = [];
        console.log("WE ARE UNDEFINED 1");
        $scope.getImageUrls = function(id){
            console.log("WE ARE IN UNDEFINED 2");
            var url = 'https://api.thenextleg.io/v2/message/' + id + '?expireMins=2'
            $http({
                method : "GET",
                url : url,
                headers: {
                ...
                if(data.progress == 'incomplete'){
                    ...
                    $scope.showLoadingApi = false;
                    return;
                }
                if(data.progress != 100){
                    $scope.apiStatus = data.progress + '%';
                    $scope.getImageUrls(id);
                }else{
                    $scope.apiStatus = '';
                    $scope.nextImages = data.response.imageUrls;
                    $scope.showLoadingApi = false;
                }
            });
        }
    ...
    $scope.initSettings = function(){
        ...
        $scope.$watchCollection('resource.clipart.filter', function(newVal, oldVal){
            ...
            console.log($scope.resource.clipart.filteredArts,11111);
            if(newVal.search != oldVal.search){
                $timeout(function() {
...
/*                 if( angular.isDefined( data.area ) ){
    ...
    if(typeof stability_nbd_js_ct != 'undefined') {
        $scope.createImageApi = function() {
            var formData = {
                prompt: jQuery('#prompt').val(),
                output_format: 'webp',
                action: 'create_image_api_stability_ct'
            };
            ...
            jQuery.ajax({
                ...
                data: formData,
                dataType: 'json',
                success: function (tada) {
                    var scope = $scope;
                    scope.addImageFromUrl(tada.src, true);
                    // scope.renderStage();
    ...
    if(typeof js_check_category_ct != 'undefined') {
        var arran = ['print design1', 'print design2', 'print design3', 'print design4', 'print design5', 'print design6', 'print design7', 'print design8'];
        var resultct = productCategories.some(item => arran.includes(item));
        if(resultct) {
            setTimeout(function(){
                jQuery("ul.menu-left li.item-resize.pod").hide();
                jQuery("ul.menu-right li.pod1").hide();

                jQuery('ul.menu-left li.item-resize2').on('click',function(){
                    alert("Text can be customized by admin");
                });

                jQuery('ul.menu-right li.pod2').on('click',function(){
                    alert("Text can be customized by admin");
                });
            },100);
        } else {
            setTimeout(function(){
                jQuery('ul.menu-left li.item-resize2').hide();
                jQuery('ul.menu-right li.pod2').hide();
            },100);
        }

        const modal = document.querySelector('.modal');
        $scope.showPreview = function(){
            jQuery('.fade').toggleClass('in');

            jQuery('#myModal .tren').removeClass('an');
            jQuery('#myModal .duoi').removeClass('hien');

        } 

        jQuery('.close').click(function () {
            jQuery('.fade').removeClass('in')
        }); 
        ...
        jQuery('.check-ct').on("change", function() {
            if (jQuery(this).is(":checked")) {
                jQuery('.ct-total_process .ct-process').removeClass('no-click');
            } else {
                jQuery('.ct-total_process .ct-process').addClass('no-click');
            }
        });
        ...
        jQuery(window).on('load', function() {
            if (localStorage.getItem('showPopup') === 'true') {
                ...
                jQuery('#myModal .tren').addClass('an');
                jQuery('#myModal .duoi').addClass('hien');
        ...
        var observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (jQuery('[ng-controller="optionCtrl"]').length > 0) {
                    var scope = angular.element(jQuery('[ng-controller="optionCtrl"]')).scope();
                    if (scope) {
                        setTimeout(function(){
                            var sumtotal = scope.convert_to_wc_price( scope.total_cart_price);
                            jQuery('.price-final_sc').text(scope.convert_to_wc_price( sumtotal ));
                            ...
                            if(data.flag == 'success'){
                                ...
                                if( type == 'saveforlater' ){
                                    ...
                                    if(NBDESIGNCONFIG.show_nbo_option == "1" && ( NBDESIGNCONFIG.task == 'new' || NBDESIGNCONFIG.task2 == 'update' ) ){
                                        ...
                                        if(typeof prt_weight_nbd_js_ct != 'undefined') {
...
//            }
    ...
    $scope.focusinp = function(event) {
        if( $scope.settings.is_mobile){
            const parentElement = event.currentTarget;
            if(jQuery(event.currentTarget).hasClass('lock-active')) return;
            const inputElement = parentElement.querySelector('.item-layer-inner .text-layer input');
            if (inputElement) {
...
/*         var _canvas = this.stages[stage_id]['canvas'];
    ...
    jQuery(document).ready(function () {
        ...
        //huy add_somchai_js
        // if(typeof add_somchai_js != 'undefined') {
            jQuery(document).on('click','.quantity .input-text',function() {
                renderHtmlPriceSize();
            });

            jQuery(document).on('click','.nbd-field-content',function() {
                renderHtmlPriceSize();
            });
        // }
