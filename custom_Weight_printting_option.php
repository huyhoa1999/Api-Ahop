...
function nbd_adop_custom_popup_wn_ft($option) {
    ?>
        <style type="text/css">
        ...
        <div id="myModal" class="modal fade nbdesigner_modal">
            <div class="modal-content">
                ...
                <div class="lietke">
                    ...
                    <div><b>Quality:</b> <span>Ensure a minimum resolution of <?php echo $option['mindpi']; ?> DPI for optimal print quality.</span></div>
...
add_filter('woocommerce_package_rates', 'modify_weight_for_shipping_calculation', 9999999999999, 2);
function modify_weight_for_shipping_calculation($rates, $package) {
    // Kiểm tra điều kiện sản phẩm hoặc giỏ hàng nếu cần thiết
    $total_weight = 0;
    
    // Tính lại trọng lượng giỏ hàng
    // foreach ($package['contents'] as $item) {
    //     $product = $item['data'];
    //     $quantity = $item['quantity'];
    //     echo "<pre>";
    //     print_r($_REQUEST['ct_weight']);
    //     echo "</pre>";
    //     die("cccc");
    //     $adjusted_weight = $product->get_weight() * $quantity * 1.5;
    //     $total_weight += $adjusted_weight;
    // }

    foreach (WC()->cart->get_cart() as $cart_item) {
        if (isset($cart_item['ct_weight'])) {
            $total_weight += $cart_item['ct_weight'] * $cart_item['quantity'];
        }
    }
...
add_filter('woocommerce_cart_item_quantity', 'custom_quantity_dropdown_cart', 10, 3);
function custom_quantity_dropdown_cart($product_quantity, $cart_item_key, $cart_item) {

    // $quantity  = nbdesigner_get_option('nbdesigner_quantity_select_cart');
    // $options = explode(",", $quantity);

    $product_id = $cart_item['product_id'];
    $product = wc_get_product( $product_id );

    $options = $cart_item['nbo_meta']['options'];
    // $option_fields = nbd_recursive_stripslashes( $options['fields'] );

    $_options = get_nbd_print_option($options['id']);
    $option_fields = unserialize($_options['fields']);

    $quantity_breaks = $option_fields['quantity_breaks'];
    $quantity = array_column($quantity_breaks, 'val');
    // $quantity = $option_fields['']

    $product_quantity_value = $cart_item['quantity'];
    $product_quantity = '<select style="width: 65px;padding: 5px;" name="cart[' . $cart_item_key . '][qty]" class="custom-quantity-select">';

    foreach ($quantity as $option) {
        $selected = ($option == $product_quantity_value) ? ' selected' : '';
        $product_quantity .= '<option value="' . $option . '"' . $selected . '>' . $option . '</option>';
    }
